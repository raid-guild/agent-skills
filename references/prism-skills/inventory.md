...
| nextcrm | CRM MCP operations | NextCRM MCP | Client/customer data privacy | rg-crm-ops | keep | Keep as narrow ops skill or merge under CRM owner. |
| portal-arcade-reporter | Portal Arcade read-only reporting | Game reporting APIs | Public/private leaderboard context | portal-arcade-reporter | keep | Clean standalone read-only skill. |
| portal-memory-publisher | Deprecated alias for portal ops | Portal APIs | Alias drift | portal-ops | deprecate | Replaced by `skills/portal-ops`. |
| portal-ops-skill | Portal CMS operations | Payload API | Invented content, project-management drift | portal-ops | keep | Migrated to `skills/portal-ops` with endpoint details split into references. |
...
| remotion-composition-checker | Remotion renderer inspection | Remotion renderer | Render diagnostics | rg-media-render-ops | merge | Media render ops reference. |
...
| workflow-audio-bucket-uploader | Upload audio to S3-compatible bucket | S3-compatible storage | Public URLs, credentials | rg-media-render-ops | merge | Media render ops reference. |
